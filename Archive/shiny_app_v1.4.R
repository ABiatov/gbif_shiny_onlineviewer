...
server = function(input, output, session) {
  ...
  sf_recieved_data <- reactive(st_intersection(sf_recieved_by_extent_data(), reaktive_bufered_polygon()) )


  # add request result to webmap
  observe({
    map %>%
      addCircleMarkers(data = sf_recieved_data(), # lng = ~Longitude, lat = ~Latitude, # add circle markers from dataframe
  ...
  output$downloadData_CSV <- downloadHandler(
    ...
    content = function(file) {
      write.csv(df_recieved_data(), file)
    }
  )
  ## Generate XLSX
  output$downloadData_XLSX <- downloadHandler(
    ...
    content = function(file) {
      write_xlsx(df_recieved_data(), file, colNames = TRUE)
    }
  )
